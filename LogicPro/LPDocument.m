...
NSString *LPDocumentCanvasSizeKey = @"canvasSize";
NSString *LPDocumentGraphicsKey = @"graphics";

// Values that are used as contexts by this class' invocation of KVO observer registration methods. See the comment near the top of LPGateView.m for a discussion of this.
static NSString *LPDocumentUndoKeysObservationContext = @"com.c-inspirations.LogicPro.undoKeys";
static NSString *LPDocumentUndoObservationContext = @"com.c-inspirations.LogicPro.undo";

// The document type names that must also be used in the application's Info.plist file. We'll take out all uses of LPDocumentOldTypeName and LPDocumentOldVersion1TypeName (and NSPDFPboardType and NSTIFFPboardType) someday when we drop 10.4 compatibility and we can just use UTIs everywhere.
static NSString *LPDocumentTypeName = @"com.c-inspirations.LogicPro";

#define METADATA_FILENAME   @"gates.metadata"
#define DATA_FILENAME       @"gates.data"
...
@interface LPDocument(LPForwardDeclarations)
...
- (id)contentsForType:(NSString *)typeName error:(NSError *__autoreleasing *)outError {
    if (self.metadata == nil || self.gates == nil) return nil;
    
    NSMutableDictionary *wrappers = [NSMutableDictionary dictionary];
    [self encodeObject:self.metadata toWrappers:wrappers preferredFilename:METADATA_FILENAME];
    [self encodeObject:self.gates toWrappers:wrappers preferredFilename:DATA_FILENAME];
...
- (NSArray *)gates {
    if (_gates == nil) {
        if (self.fileWrapper != nil) {
            _gates = [self decodeObjectFromWrapperWithPreferredFilename:DATA_FILENAME];
        } else {
            _gates = [NSArray array];
        }
    }
    return _gates;
}
...
//    CGSize canvasSize = CGSizeMake(500, 500);  // [printInfo paperSize];
...
//    
//    // This application's Info.plist only declares two document types, which go by the names LPDocumentOldTypeName/LPDocumentOldVersion1TypeName (on Mac OS 10.4) or LPDocumentNewTypeName/LPDocumentNewVersion1TypeName (on 10.5), for which it can play the "editor" role, and none for which it can play the "viewer" role, so the type better match one of those. Notice that we don't compare uniform type identifiers (UTIs) with -isEqualToString:. We use -[NSWorkspace type:conformsToType:] (new in 10.5), which is nearly always the correct thing to do with UTIs.
//    BOOL readSuccessfully;
//    NSArray *graphics = nil;
//    UIPrintInfo *printInfo = nil;
////    NSWorkspace *workspace = [NSWorkspace sharedWorkspace];
//    BOOL useTypeConformance = NO; // [workspace respondsToSelector:@selector(type:conformsToType:)];
//    if ((useTypeConformance && [workspace type:typeName conformsToType:LPDocumentNewTypeName]) || [typeName isEqualToString:LPDocumentNewTypeName]) {
//        
...
////            NSArray *graphicPropertiesArray = [properties objectForKey:LPDocumentGraphicsKey];
////            graphics = [graphicPropertiesArray isKindOfClass:[NSArray class]] ? [LPGate graphicsWithProperties:graphicPropertiesArray] : [NSArray array];
////            
...
////            NSData *printInfoData = [properties objectForKey:LPDocumentPrintInfoKey];
...
////            *outError = LPErrorWithCode(LPUnknownFileReadError);
////            
////        }
//        readSuccessfully = properties ? YES : NO;
//        
//    } else {
//        NSParameterAssert((useTypeConformance && [workspace type:typeName conformsToType:LPDocumentNewVersion1TypeName]) || [typeName isEqualToString:LPDocumentNewVersion1TypeName]);
//        
//        // The file uses Sketch's old format. Sketch is still a work in progress.
//        graphics = [NSArray array];
//        printInfo = [[UIPrintInfo alloc] init];
...
//    // - LPDocumentOldTypeName (on Mac OS 10.4) or LPDocumentNewTypeName (on 10.5), because this application's Info.plist file declares that instances of this class can play the "editor" role for it, and
...
//    // If we had reason to believe that +[LPRenderingView pdfDataWithGraphics:] or +[LPGate propertiesWithGraphics:] could return nil we would have to arrange for *outError to be set to a real value when that happens. If you signal failure in a method that takes an error: parameter and outError!=NULL you must set *outError to something decent.
//    NSData *data;
//    NSArray *graphics = [self graphics];
//    NSPrintInfo *printInfo = [self printInfo];
//    NSWorkspace *workspace = [NSWorkspace sharedWorkspace];
//    BOOL useTypeConformance = [workspace respondsToSelector:@selector(type:conformsToType:)];
//    if ((useTypeConformance && [workspace type:LPDocumentNewTypeName conformsToType:typeName]) || [typeName isEqualToString:LPDocumentOldTypeName]) {
//        
//        // Convert the contents of the document to a property list and then flatten the property list.
//        NSMutableDictionary *properties = [NSMutableDictionary dictionary];
//        [properties setObject:[NSNumber numberWithInteger:LPDocumentCurrentVersion] forKey:LPDocumentVersionKey];
//        [properties setObject:[LPGate propertiesWithGraphics:graphics] forKey:LPDocumentGraphicsKey];
//        [properties setObject:[NSArchiver archivedDataWithRootObject:printInfo] forKey:LPDocumentPrintInfoKey];
...
//        data = [LPRenderingView pdfDataWithGraphics:graphics];
//    } else {
...
//        data = [LPRenderingView tiffDataWithGraphics:graphics error:outError];
//    }
//    return data;
//    
//}
...
//    [self willChangeValueForKey:LPDocumentCanvasSizeKey];
//    [super setPrintInfo:printInfo];
//    [self didChangeValueForKey:LPDocumentCanvasSizeKey];
//    
//}
...
//    LPRenderingView *renderingView = [[LPRenderingView alloc] initWithFrame:NSMakeRect(0.0, 0.0, documentSize.width, documentSize.height) graphics:[self graphics] printJobTitle:printJobTitle];
//    
...
//    LPWindowController *windowController = [[LPWindowController alloc] init];
...
//
//- (void)setGraphicProperties:(LPMapTableOwner *)propertiesPerGraphic {
//    
...
//            [graphic addObserver:self forKeyPath:key options:(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:LPDocumentUndoObservationContext];
//            
//        }
//        
//        // The set of properties to be observed can itself change.
//        [graphic addObserver:self forKeyPath:LPGateKeysForValuesToObserveForUndoKey options:(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:LPDocumentUndoKeysObservationContext];
//        
//    }
//    
//}
...
//    if (context==LPDocumentUndoKeysObservationContext) {
//        
...
//                [observedObject addObserver:self forKeyPath:key options:(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:LPDocumentUndoObservationContext];
//            }
//        }
//        
//    } else if (context==LPDocumentUndoObservationContext) {
//        
...
//        LPGate *graphic = (LPGate *)observedObject;
//        if (![_undoGroupInsertedGraphics containsObject:graphic]) {
//            
//            // Ignore changes that aren't really changes. Now that Sketch's inspector panel allows you to change a property of all selected graphics at once (it didn't always, as recently as the version that appears in Mac OS 10.4's /Developer/Examples/AppKit), it's easy for the user to cause a big batch of LPGates to be sent -setValue:forKeyPath: messages that don't do anything useful. Try this simple example: create 10 circles, and set all but one to be filled. Select them all. In the inspector panel the Fill checkbox will show the mixed state indicator (a dash). Click on it. Cocoa's bindings machinery sends [theCircle setValue:[NSNumber numberWithBOOL:YES] forKeyPath:LPGateIsDrawingFillKey] to each selected circle. KVO faithfully notifies this LPDocument, which is observing all of its graphics, for each circle object, even though the old value of the LPGateIsDrawingFillKey property for 9 out of the 10 circles was already YES. If we didn't actively filter out useless notifications like these we would be wasting memory by recording undo operations that don't actually do anything.
...
//                    
//                    // We have to create a dictionary to hold old values for the changed graphic. -[NSMutableDictionary setObject:forKey:] always makes a copy of the key object, but we don't want to make copies of LPGates here, so we can't use NSMutableDictionary. That's why _undoGroupOldPropertiesPerGraphic uses NSMapTable despite the hassle of having to wrap it in LPMapTableOwner.
...
//                }
//                
//                // Don't set the undo action name during undoing and redoing. In Sketch, LPGateView sometimes overwrites whatever action name we set up here with something more specific (as in, "Move" or "Resize" instead of "Change of Bounds"), but only during the building of the original undo action. During undoing and redoing LPGateView doesn't get a chance to do that desirable overwriting again. Just leave the action name alone during undoing and redoing and the action name from the original undo group will continue to be used.
...
//                        
//                        // Someone overrode -[LPGate keysForValuesToObserveForUndo] but didn't override +[LPGate presentablePropertyNameForKey:] to match. Help debug a little. Hopefully the LPGate public interface makes it so that you only have to test a little bit to find bugs like this.
...
//    if (objectClass==[LPGate class]) {
//        objectClass = [LPCircle class];
//    }
...
//- (void)insertGraphic:(LPGate *)graphic atIndex:(NSUInteger)index {
//    
...
//
//
//- (void)addInGraphics:(LPGate *)graphic {
//    
...
//
//- (NSArray *)rectangles {
//    return [NSArray array];  // [self graphicsWithClass:[LPRectangle class]];
//}
//
//- (NSArray *)circles {
//    return [NSArray array];  // [self graphicsWithClass:[LPCircle class]];
//}
//
//- (NSArray *)lines {
//    return [NSArray array];  // [self graphicsWithClass:[LPLine class]];
//}
//
//- (NSArray *)textAreas {
//    return [NSArray array];  // [self graphicsWithClass:[LPText class]];
//}
//
//- (NSArray *)images {
//    return [NSArray array];  // [self graphicsWithClass:[LPImage class]];
//}
//
//- (void)insertObject:(LPGate *)graphic inRectanglesAtIndex:(NSUInteger)index {
...
//}
//
//- (void)insertObject:(LPGate *)graphic inCirclesAtIndex:(NSUInteger)index {
...
//}
//
//- (void)insertObject:(LPGate *)graphic inLinesAtIndex:(NSUInteger)index {
...
//}
//
//- (void)insertObject:(LPGate *)graphic inTextAreasAtIndex:(NSUInteger)index {
...
//}
//
//- (void)insertObject:(LPGate *)graphic inImagesAtIndex:(NSUInteger)index {
...
// The following "indicesOf..." methods are in support of scripting.  They allow more flexible range and relative specifiers to be used with the different graphic keys of a LPDocument.
